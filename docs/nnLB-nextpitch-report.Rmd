...
glasnow_auc <- ggplot(glasnow_auc_data) +
  ...
  scale_x_continuous(expand=c(0, 0), limits = c(0, 1), breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1)) +
...
glasnow_auprc <- ggplot(glasnow_auprc_data) +
  ...
  geom_segment(
    ...
    linetype = "dashed", lwd = 0.2, color = "gray60") +
  scale_x_continuous(expand=c(0, 0), limits = c(0, 1), breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1)) +
  scale_y_continuous(expand=c(0, 0), limits = c(0, 1), breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1)) +
  scale_color_manual(
    values = c("#86BBD8", "#758E4F", "#F6AE2D", "#F26419"),
...
```
```{r, fig.show = 'hold', out.width = "70%", fig.cap = "Figure 4. Multiclass pitch classification metric visualization for Walker Buehler. One-vs-rest metrics are stratified on the x-axis by pitch type. Pitch types include fastball (FB), fastball with movement (FBwM), and off-speed (OFF)."}
